...
public class OPTICS {
	...
	public double max_distance; // this is the epselon in the description
	public int min_points; // minimum number of points in the cluster (should be 2)
	...
	public void cluster() {
		...
		while(it.hasNext()) {
			...
			if(!processed[n]) {
				processed[n] = true;
				List neighbors = getNeighbors(point,n);
				order.add(point);
				if(core_distance[n] != -1) {
					...
					for(Pair<Integer,Double> seed : seeds) {
						...
						List seedNeighbors = getNeighbors(seedPoint,node);
						order.add(seedPoint);
	...
	private void update(DataPoint point, List<Pair<Integer, Double>> seeds,
	...
	private List getNeighbors(DataPoint d, int index) {
